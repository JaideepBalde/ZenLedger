...
const App: React.FC = () => {
  ...
  const toggleMusic = () => {
    if (!audioRef.current) {
      audioRef.current = new Audio('https://archive.org/download/GayatriMantra_201605/Gayatri%20Mantra.mp3');
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] relative overflow-x-hidden">
          ...
          <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
            <div className="max-w-7xl mx-auto px-8 py-5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-lg font-black tracking-tighter italic">{BRAND_NAME}</h1>
              ...
              <button onClick={() => { storageService.logout(); setSession(null); }} className="px-5 py-2 bg-slate-100 hover:bg-rose-50 hover:text-rose-600 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">Disconnect</button>
            </div>
          </header>

          <main className="max-w-7xl mx-auto px-8 mt-10">
            <div className="flex bg-white rounded-2xl p-1 border border-slate-200 mb-10 shadow-sm max-w-sm">
              {['LEDGER', 'ANALYTICS', 'MESSAGES'].map((t: any) => (
                <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === t ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>{t}</button>
              ))}
            </div>
              ...
              {activeTab === 'LEDGER' && (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                   <div className="lg:col-span-4 space-y-8">
                      ...
                      <div className="bg-white border border-slate-200 rounded-[2rem] p-10 space-y-5">
                         <h3 className="text-[9px] font-black uppercase text-slate-400 mb-6 tracking-widest">Execute DEBIT</h3>
                         <div className="space-y-4">
                           ...
                           }} className="w-full py-4.5 bg-slate-900 text-white rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-black transition-all">Log Disbursement</button>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="lg:col-span-8">
                      <div className="bg-white border border-slate-200 rounded-[2rem] p-12">
                         <h3 className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-10 italic">Audit Ledger</h3>
                         <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
                            {txs.filter(t=>t.userId===session.userId).map(tx => (
                              <div key={tx.id} className="flex items-center justify-between p-6 bg-slate-50 border border-slate-100 rounded-2xl hover:border-slate-900 transition-all group">
                                <div className="flex items-center gap-5">
                                   <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${tx.type === TransactionType.CREDIT ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-200 text-slate-500'}`}>
                                      {tx.type === TransactionType.CREDIT ? '↓' : '↑'}
                                   </div>
                                   <div>
                                      <p className="text-sm font-bold text-slate-900 leading-tight tracking-tight">{tx.description}</p>
                                      <p className="text-[8px] text-slate-400 font-black uppercase tracking-widest mt-1.5">{new Date(tx.timestamp).toLocaleString()}</p>
                                   </div>
                                </div>
                                <div className={`text-xl font-black tabular-nums tracking-tighter ${tx.type === TransactionType.CREDIT ? 'text-emerald-600' : 'text-slate-900'}`}>
                                  {tx.type === TransactionType.CREDIT ? '+' : '-'}{tx.amount.toFixed(2)}
                                </div>
                              </div>
                            ))}
                            {txs.filter(t=>t.userId===session.userId).length === 0 && <p className="text-center py-40 text-slate-300 font-black uppercase text-[10px] tracking-widest italic">Ledger Clean</p>}
                         </div>
                      </div>
                   </div>
                </div>
      ...
      <div className="fixed bottom-10 right-10 z-[200]">
        <button 
          onClick={toggleMusic}
          className={`w-14 h-14 rounded-full flex items-center justify-center text-3xl shadow-xl transition-all duration-700 border-2 ${isMusicPlaying ? 'bg-slate-900 text-white border-white animate-pulse' : 'bg-white text-slate-900 border-slate-200 hover:scale-110 active:scale-95'}`}
